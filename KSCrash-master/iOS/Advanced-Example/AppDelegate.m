...
#import <KSCrash/KSCrashInstallationFileStore.h>
...
//    self.crashInstallation = [self makeVictoryInstallation];
    self.crashInstallation = [self makeFileStoreInstallation];
...
- (KSCrashInstallation*)makeFileStoreInstallation
{
    NSString *fileName = [NSString stringWithFormat:@"%lf",[[NSDate date]timeIntervalSince1970]];
    
    NSString *fileMeta = @"KSCrash 文件信息头";
    
    NSArray *cachePaths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    NSString *cachePath = [[cachePaths lastObject]stringByAppendingPathComponent:fileName];
    
    NSLog(@"crash 文件缓存路径:%@",cachePath);
    
    KSCrashInstallationFileStore *fileStore = [KSCrashInstallationFileStore sharedInstance];
    fileStore.filePath = cachePath;
    fileStore.fileMeta = fileMeta;
